...
Androdex has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.androdex/userdata/logs/server.trace.ndjson`).
...
```bash
export ANDRODEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export ANDRODEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export ANDRODEX_OTLP_SERVICE_NAME=androdex-local
```

Optional:

```bash
export ANDRODEX_TRACE_MIN_LEVEL=Info
export ANDRODEX_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `ANDRODEX_OTLP_*`.

macOS app bundle example:

```bash
ANDRODEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
ANDRODEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
ANDRODEX_OTLP_SERVICE_NAME=androdex-desktop \
"/Applications/Androdex.app/Contents/MacOS/Androdex"
```
...
```bash
tail -f "$ANDRODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$ANDRODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$ANDRODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `androdex-local`, `androdex-dev`, or `androdex-desktop`
...
These metric IDs intentionally keep the legacy `t3_` prefix for observability continuity. The
service names, app identity, and user-facing branding are Androdex, but dashboards and alerting can
continue to rely on the existing metric series names.
...
- `ANDRODEX_OTLP_TRACES_URL` was not set
...
- `ANDRODEX_TRACE_FILE`: override trace file path
- `ANDRODEX_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `ANDRODEX_TRACE_MAX_FILES`: rotated file count, default `10`
- `ANDRODEX_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `ANDRODEX_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `ANDRODEX_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `ANDRODEX_OTLP_TRACES_URL`: OTLP trace endpoint
- `ANDRODEX_OTLP_METRICS_URL`: OTLP metric endpoint
- `ANDRODEX_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `ANDRODEX_OTLP_SERVICE_NAME`: service name, default `androdex-server`
