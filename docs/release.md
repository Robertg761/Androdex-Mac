...
- Publishes the CLI package (`apps/server`, npm package `androdex`) with OIDC trusted publishing from the same workflow file:
...
- Repository slug source:
  - `ANDRODEX_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `ANDRODEX_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `androdex`.
