...
import {
  ...
  resolveRemoteAndrodexCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote Androdex package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteAndrodexCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "androdex@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteAndrodexCliPackageSpec({
