...
const PUBLISHABLE_ANDRODEX_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteAndrodexCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_ANDRODEX_VERSION_PATTERN.test(appVersion)) {
    return `androdex@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "androdex@nightly";
  }

  return input.updateChannel === "nightly" ? "androdex@nightly" : "androdex@latest";
}
