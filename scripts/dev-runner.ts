...
export const DEFAULT_ANDRODEX_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".androdex"),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "dev", "--filter=androdex"],
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_ANDRODEX_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (mode === "dev") {
      ...
      delete output.ANDRODEX_DESKTOP_WS_URL;
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.ANDRODEX_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.ANDRODEX_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  baseDir: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Androdex data (equivalent to ANDRODEX_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("ANDRODEX_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to ANDRODEX_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("ANDRODEX_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to ANDRODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("ANDRODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to ANDRODEX_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("ANDRODEX_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to ANDRODEX_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("ANDRODEX_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to ANDRODEX_PORT)."),
