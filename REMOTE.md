...
- HTTPS/WSS-compatible endpoints work from `https://app.androdex.xyz`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.androdex.xyz/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
with the same server-side behavior as `androdex serve --tailscale-serve`, then the server asks Tailscale
...
For `https://app.androdex.xyz`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
```bash
npx androdex serve --tailscale-serve
```
...
```bash
npx androdex serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse Androdex on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Androdex server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Androdex server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.androdex/ssh-launch/<host-key>/`, starts or reuses a remote Androdex server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Androdex uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Androdex first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Androdex uses:
...
The hosted web app at `https://app.androdex.xyz` can save a remote backend in browser local storage from a URL like:

```text
https://app.androdex.xyz/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through Androdex. The browser still connects directly to the backend URL in the pairing link.
