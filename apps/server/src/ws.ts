...
const makeWsRpcLayer = (currentSessionId: AuthSessionId) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const shouldStopSessionBeforeArchive = (threadId: ThreadId) =>
        projectionSnapshotQuery.getThreadShellById(threadId).pipe(
          ...
          Effect.catch((error) =>
            Effect.logWarning("failed to check thread session before archive", {
              threadId,
              error: error.message,
      ...
      const stopThreadSessionAfterArchive = (threadId: ThreadId) =>
        dispatchNormalizedCommand({
          type: "thread.session.stop",
          commandId: serverCommandId("archive-session-stop"),
          threadId,
          createdAt: new Date().toISOString(),
        }).pipe(Effect.ignoreCause({ log: true }), Effect.asVoid);
            ...
            Effect.gen(function* () {
              ...
              const shouldStopSession =
                normalizedCommand.type === "thread.archive"
                  ? yield* shouldStopSessionBeforeArchive(normalizedCommand.threadId)
                  : false;
              const result = yield* dispatchNormalizedCommand(normalizedCommand);
              if (normalizedCommand.type === "thread.archive") {
                if (shouldStopSession) {
                  yield* stopThreadSessionAfterArchive(normalizedCommand.threadId);
                }
