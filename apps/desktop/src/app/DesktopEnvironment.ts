...
const makeDesktopEnvironment = Effect.fn("desktop.environment.make")(function* (
  ...
  const baseDir = Option.getOrElse(config.t3Home, () =>
    path.join(homeDirectory, DEFAULT_BASE_DIR_NAME),
  );
  ...
  const userDataDirName = isDevelopment ? `${PRODUCT_SLUG}-dev` : USER_DATA_DIR_NAME;
  ...
  return DesktopEnvironment.of({
    ...
    branding,
    displayName,
    appUserModelId: isDevelopment ? `${APP_BUNDLE_ID}.dev` : APP_BUNDLE_ID,
