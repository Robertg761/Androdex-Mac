...
describe("incremental orchestration updates", () => {
  ...
  it("ignores stale thread.session-set snapshots so completed turns do not regress to running", () => {
    const thread = makeThread({
      session: {
        provider: "codex",
        status: "ready",
        ...
        createdAt: "2026-02-27T00:00:06.000Z",
    ...
    expect(threadsOf(next)[0]?.session?.updatedAt).toBe("2026-02-27T00:00:06.000Z");
    expect(threadsOf(next)[0]?.latestTurn).toEqual(thread.latestTurn);
  });

  it("ignores stale thread.session-set snapshots so active running state does not regress to ready", () => {
    const thread = makeThread({
      session: {
        provider: "codex",
        status: "running",
      ...
      latestTurn: {
        ...
        state: "running",
        requestedAt: "2026-02-27T00:00:06.000Z",
        startedAt: "2026-02-27T00:00:06.000Z",
        completedAt: null,
        assistantMessageId: null,
      },
    });
