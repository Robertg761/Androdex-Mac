...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const createThreadForProjectMember = useCallback(
    ...
    [closeMobileSidebar, defaultThreadEnvMode, handleNewThread, router],
  );
...
const SidebarChromeHeader = memo(function SidebarChromeHeader({
  ...
  return isElectron ? (
    <SidebarHeader
      className="drag-region h-[52px] flex-row items-center gap-2 px-4 py-0 pl-[90px] wco:h-[env(titlebar-area-height)] wco:pl-[calc(env(titlebar-area-x)+1em)]"
