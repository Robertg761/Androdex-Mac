...
This document describes the target architecture for first-class remote environments in Androdex.
...
- Keep the Androdex server as the execution boundary.
...
Androdex already has a clean runtime boundary: the client talks to a Androdex server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Androdex server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Androdex server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Androdex runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Androdex server instance.
...
How does the client speak WebSocket to a Androdex server?
...
- best fit for hosted or self-managed remote Androdex deployments
...
For Androdex, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Androdex-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Androdex server
...
How does a Androdex server come to exist on the target machine?
...
The user or operator already runs Androdex on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Androdex:
...
The recommended Androdex flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Androdex availability.
3. Desktop launches or reuses a remote Androdex server.
...
This is the inverse of remote launch: a local Androdex server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Androdex server might be reached through direct `wss`.
...
Androdex already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Androdex should not copy that part.

Androdex already has the right runtime boundary:

- one Androdex server per environment
- ordinary HTTP/WebSocket between client and environment

So Androdex should borrow Zed's launch discipline, not its transport protocol.
