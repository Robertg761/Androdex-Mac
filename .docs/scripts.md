...
- Dev commands default `ANDRODEX_STATE_DIR` to `~/.androdex/dev` to keep dev state isolated from desktop/prod state.
...
- Desktop production windows load the bundled UI from `androdex://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `androdex` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `ANDRODEX_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `ANDRODEX_DEV_INSTANCE`)
- Example: `ANDRODEX_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `ANDRODEX_PORT_OFFSET` to a numeric offset.
